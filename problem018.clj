...
(def triangle
  (reverse (map (fn [row] (map #(Integer/parseInt %) (split #"[ ]" row)))
		["75"
		 "95 64"
		 "17 47 82"
		 "18 35 87 10"
...
(defn find-maximum-total [triangle]
  (loop [[row & rows] (rest triangle),
	 so-far (first triangle)]
    (if (nil? row)
      (first so-far)
      (recur rows
	     (map #(+ (max %1 %2) %3)
